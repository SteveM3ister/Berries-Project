...
bberry%<>%separate(meas,c("Status","Process"),sep="-")

for(i in 1:length(bberry$Year)){
  if(str_detect(bberry$lab2[i], "HAR")){
    bberry$Status[i]=bberry$lab2[i]
  }
}
bberry%<>%select(-lab2)
for(i in 1:length(bberry$Year)){
  if(str_detect(bberry$lab1[i], "ED")){
    bberry$Status[i]=bberry$lab1[i]
    bberry$lab1[i]=" "
  }
  else if(str_detect(bberry$lab1[i],"ACRES")){
    bberry$what[i]=bberry$lab1[i]
    bberry$lab1[i]=" "
  }
  else if(bberry$lab1[i]!=" "){
    bberry$Process[i]=bberry$lab1[i]
    bberry$lab1[i]=" "
  }
}


#Check if lab1 is cleaned
sum(str_detect(bberry$lab1, " ")) == length(bberry$lab1)
bberry%<>%select(-lab1)
#Elimate NA's
bberry[is.na(bberry)] <- " "
#Now the dataset is cleaned.
bberry%<>%rename(Measure=what,Chem=D_left,killer=D_right,killer_info=DC_left_r,Compound=DC_right_r)
